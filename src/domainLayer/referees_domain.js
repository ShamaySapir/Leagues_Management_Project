...
async function insertToRefreeTable(user) {
  ...
  if (referee_exist !== -1) {
    console.error("Referee already exist");
    throw error;
  }
  let referee_added = await referee_utils.insertRefereeInfo(
    userId,
    user.username
  );
...
async function assignRefereeToMatch(refereeId, matchId) {
  if (matchId == null) {
    console.error("matchId required");
    throw error;
  }
...
async function getReferees() {
  referees = await referee_utils.getReferees();
  return referees;
}
